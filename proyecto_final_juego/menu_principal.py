...
class Menu_princial():
    ...
    def setup_form_menu(self,path_buttons):
        for path in path_buttons:
            button = Boton(path="{0}".format(PATH_MENU),size=(150,75),frame=path,master_screen=self.surface_menu,pos=self.rect_menu.center)
    ...
    def update(self,event):
        ...
        print(self.rect_menu.centerx)
        for button in self.buttons_list:
            print(button.rect_button.center)
            button.draw()
    def draw(self,event):
        self.update(event)
